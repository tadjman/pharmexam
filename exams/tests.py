...
class SessionAndExamFilterTests(TestCase):
    ...
    def test_session_list_filters_by_query(self):
        response = self.client.get(reverse("exams:session_list"), {"q": "Janv"})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Session Janvier")
        self.assertNotContains(response, "Session Juin")

    def test_exam_list_filters_by_query(self):
        response = self.client.get(reverse("exams:exam_list"), {"q": "Toxico"})
        self.assertEqual(response.status_code, 200)
...
class ExamenValidationMessageTests(TestCase):
    def test_responsable_error_is_attached_to_field(self):
        teacher = User.objects.create_user(
            username="teacher_validation_exam",
            password="pass",
            role=RoleUtilisateur.ENSEIGNANT,
        )
        outsider = User.objects.create_user(
            username="outsider_validation_exam",
            password="pass",
            role=RoleUtilisateur.ENSEIGNANT,
        )
